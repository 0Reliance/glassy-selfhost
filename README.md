...
#   GLASSY_TAG=<latest released version, e.g. v2.36.0-beta.40>  (see note below)
...
| `GLASSY_TAG` | Image tag to pull from GHCR — **required** (set in `.env`): must name a released version (e.g. `v2.36.0-beta.40`), never `latest` (the hosted build omits self-host features). |
